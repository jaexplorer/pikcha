...
namespace PikchaWebApp.Test.Unit
{
    public class FilterControllerTest : IClassFixture<SqliteInMemoryFixture>
    {
        ...
        public async Task Get_ReturnsJson_ListofImages(string type, int start, int count, int expectedStatus)
        {
            ...
            if (type == "pikcha100")
            {
                var imlst = response.Value as List<Pikcha100ImageDTO>;
                // validate uploaded image
                Assert.True(imlst.Exists(i => i.Title == img.Title));

                Assert.True(File.Exists(imlst[0].ThumbnailFile));
                Assert.True(File.Exists(imlst[0].WatermarkedFile));
                return;
            }

            var lst = response.Value as List<PikchaRandomImageDTO>;
            // validate uploaded image
            Assert.True(lst.Exists(i => i.Title == img.Title));

            Assert.True(File.Exists(lst[0].ThumbnailFile));
            Assert.True(File.Exists(lst[0].WatermarkedFile));
        ...
        public async Task Get_ReturnsJson_ListofArtists(string type, int start, int count, int expectedStatus)
        {
            ...
            if (type == "artists100")
            {
                var imlst = response.Value as List<Pikcha100ArtistDTO>;
                // validate uploaded image
                Assert.True(imlst.Exists(i => i.Email == email1));
                return;
            }

            var lst = response.Value as List<Pikcha100ArtistDTO >;
            // validate uploaded image
